...
function validate(config) {
  ...
  if (!Number.isInteger(config.nodes) || config.nodes < 1 || config.nodes > inquirer.maxValidators) {
    console.error(chalk.red(`'nodes' must be an integer in the range [1, ${inquirer.maxValidators}]`));
    return false;
  }
  return true;
}
