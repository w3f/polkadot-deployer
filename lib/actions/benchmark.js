...
function validate(config) {
  ...
  if (config.benchmark === 'finality') {
    ...
    if (config.blocks.warm && !Number.isInteger(config.blocks.measure)) {
      console.error(chalk.red(`'blocks.warm' field must be an integer`));
      return false;
    }
  }
  return true;
...
async function benchmark(config, output) {
  ...
  const benchmarkFile = files.writeBenchmarks(config, results, output);
...
async function shouldCreate(config) {
  const exists = await cluster.exists(config);
