...
module.exports = {
  ...
  createLocal: async (config) => {
    try {
      await cmd.exec(`docker run -d --privileged --name ${config.name} -p 8443:8443 -p 10080:10080 bsycorp/kind:latest-1.13`, {verbose: config.verbose});
    } catch (err) {
  ...
  exists: async (config) => {
    if (config.type === 'local') {
      const containerList = await cmd.exec(`docker ps`, {verbose: config.verbose});
      const pattern = new RegExp(`\s*${config.name}\s*`);
      return pattern.test(containerList.toString());
    }
  }
}
