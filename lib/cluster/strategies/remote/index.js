...
class RemoteCluster {
  constructor(config) {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) {
      const name = nameLib.generate(this.config.name, counter); //FIXME - explicit literal
      this._initializeTerraform(counter); //FIXME - explicit literal
      this.options.push({
  ...
  async create() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) { //FIXME - explicit literal
      ...
      creationPromises.push(new Promise(async (resolve) => {
        ...
        await this._saveKubeconfig(counter, name, false); //FIXME - explicit literal
        resolve(true);
      }));
    }
  ...
  setKeys(keys) {
    ...
    for (let counter = 0; counter < this.helm.length; counter++) {
      this.helm[counter].setKeys(keys); //FIXME - explicit literal
    }
  }
  ...
  async installNodes() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) { //FIXME - explicit literal
      ...
      for (let index = 0; index < counter; index++) { //FIXME - explicit literal
      ...
      const startKeyIndex = startNode - (counter * nonValidatorsPerCluster); //FIXME - explicit literal
      ...
      const nodeKeys = this.config.nodeKeys.slice(startNode, endNode + 1); //FIXME - explicit literal
  ...
  async installDeps() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) {
      installDepsPromise.push(this.helm[counter].installDeps(counter)); //FIXME - explicit literal
    }
  ...
  async deleteNodes() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) {
      deleteNodesPromise.push(this.helm[counter].delete()); //FIXME - explicit literal
    }
  ...
  async getConfig() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) {
      ...
      getConfigPromises.push(new Promise(async (resolve) => {
        await this._saveKubeconfig(counter, name); //FIXME - explicit literal
        resolve(true);
      }));
    }
  ...
  _domain(index) { //FIXME - explicit literal
  ...
  _bootNodes() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) { 
      ...
      const p2pDomain = this._p2pDomain(counter); //FIXME - explicit literal
      const peerId = this.config.nodeKeys[bootNodeIndex].peerId; //FIXME - explicit literal
