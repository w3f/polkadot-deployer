...
class RemoteCluster {
  ...
  async create() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) {
      ...
      creationPromises.push(new Promise(async (resolve) => {
        ...
        resolve(this.files.write(targetKubeconfigPath, kubeconfigContent));
      }));
    }
  ...
  async installNodes() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) {
      ...
      const validatorBootnodes = this._validatorBootnodes(counter);
  ...
  _validatorBootnodes(index) {
    const validatorBootnodeCfg = this.config.remote.clusters[index].validatorBootnode;

    if (!validatorBootnodeCfg) {
      return [];
    }

    const domain = this._p2pVDomain(validatorBootnodeCfg.index);
    const port = constants.initialP2PPort + validatorBootnodeCfg.index;
    const peerId = this.config.nodeKeys[validatorBootnodeCfg.index].peerId;
