...
class RemoteCluster {
  ...
  async destroy() {
    ...
    try {
      ...
      console.log(`Could not delete clusters: ${e.message}`);
    }
  ...
  _initializeTerraform(index) {
    ...
    this.files.copyTerraformFiles(clusterName, this.config.type);

    const terraformTplPath = this.files.terraformTplPath(this.config.type);
