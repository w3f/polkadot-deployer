...
class LocalCluster {
  ...
  async create() {
    ...
    const sourceKubeconfigPathBuffer = await this._cmd(`get kubeconfig-path --name=${this.config.name}`);
    const sourceKubeconfigPath = sourceKubeconfigPathBuffer.toString().trim();
    const targetKubeconfigPath = this.files.kubeconfigPath(this.config.name);
    this.files.copy(sourceKubeconfigPath.trim(), targetKubeconfigPath)
  }
  ...
  setKeys(keys) {
    ...
    this.helm.setSessionKeys(keys['session']);
  }
  ...
  async installNodes() {
    ...
    this.helm.setSessionKeys(this.config.keys['session']);
