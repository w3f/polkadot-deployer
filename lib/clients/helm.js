...
class Helm {
  ...
  _generateValuesFile(index) {
    ...
    if (this.config.remote.monitoring) {
      values.telemetry = domain.telemetry(this.config.deploymentName, this.config.remote.clusters[0].domain);
    }
    if (index === 0) {
  ...
  _generateValuesFileBase(index) {
    ...
    if (this.config.type === 'local' ) {
      ...
      values.domain = domain.default(this.config.name, this.config.remote.clusters[index].domain);
      values.bootNodeP2PPort = this._p2pPort(0);
      values.cloudflareEmail = this.cloudflareEmail;
      values.monitoring = this.config.remote.monitoring;
    }
    return values;
  }
  ...
  async _installNgixIngress() {
    await this._cmd(`upgrade --install nginx-ingress --set controller.publishService.enabled=true --version ${this.mainConfig.chartVersions['nginx-ingress']} stable/nginx-ingress`);
  }

  async _installExternalDNS() {
    const chart = 'external-dns';
    files.copyChartValuesFiles(this.config.name, chart);
    const chartValuesPath = files.chartValuesPath(this.config.name, chart);
  ...
  async _installMatrixbot() {
    await this._cmd(`upgrade --install matrixbot --set botUser=${this.botUser} --set botPassword=${this.botPassword} --set roomId=${this.botRoomId} --version ${this.mainConfig.chartVersions['matrixbot']} w3f/matrixbot`);
  }

  async _installSubstrateTelemetry() {
    const telemetryDomain = domain.default(this.config.name, this.config.remote.clusters[0].domain);
    await this._cmd(`upgrade --install substrate-telemetry --set domain=${telemetryDomain} --version ${this.mainConfig.chartVersions['substrate-telemetry']} w3f/substrate-telemetry`);
  }
  ...
  async _installPolkadotSecrets() {
    ...
    const relativeValuesFilePath = files.relativeValuesFilePath(this.config.name, 'secrets');
    const fullValuesFilePath = files.absoluteValuesFilePath(this.config.name, 'secrets');
    files.writeYAML(fullValuesFilePath, values);
    const baseVersion = this.mainConfig.chartVersions.polkadot;
