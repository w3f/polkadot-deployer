...
class Helm {
  constructor(config) {
    ...
    this.cloudflareEmail = process.env.CLOUDFLARE_EMAIL || '';
    this.opsgenieEnabled = process.env.OPSGENIE_ENABLED || false;
    this.opsgenieURL = process.env.OPSGENIE_URL || 'https://api.eu.opsgenie.com/v2/alerts';
    this.opsgenieToken = process.env.OPSGENIE_TOKEN || '';
  ...
  async _installPrometheusOperator() {
    if(${this.opsgenieEnabled}){
      await this._cmd(`upgrade --install prometheus-operator --set opsgenie.enabled=${this.opsgenieEnabled} --set opsgenie.token=${this.opsgenieToken} --set opsgenie.url=${this.opsgenieUrl} --version ${this.mainConfig.chartVersions['prometheus-operator']} stable/prometheus-operator`);
    }
    else {
