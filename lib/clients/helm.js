...
class Helm {
  ...
  async installDeps(index=0, namespace='default') {
    ...
    if (this.config.type !== 'local') {
      ...
      if (!this.config.benchmark) {
        ...
        await this._installCertManager(namespace);

        if (this.config.remote.monitoring) {
  ...
  _generateValuesFile(index) {
    ...
    if (this.config.type !== 'local' && this.config.remote.monitoring) {
  ...
  _generateValuesFileBase(index) {
    ...
    if (this.config.type === 'local' ) {
      ...
      values.monitoring = this.config.remote.monitoring || false;
  ...
  async _installCertManager(namespace) {
    ...
    await this._cmd(`upgrade --install --atomic cert-manager --namespace cert-manager --version ${this.mainConfig.chartVersions['cert-manager']} jetstack/cert-manager`);
  }

  async _installNgixIngress() {
    await this._cmd(`upgrade --install --atomic nginx-ingress --set controller.publishService.enabled=true --set controller.config.use-http2=false --version ${this.mainConfig.chartVersions['nginx-ingress']} stable/nginx-ingress`);
  }
  ...
  async _installExternalDNS() {
    ...
    await this._cmd(`upgrade --install --atomic ${chart} --set cloudflare.apiKey=${this.cloudflareApiKey} --set cloudflare.email=${this.cloudflareEmail} -f ${chartValuesPath} --version ${this.mainConfig.chartVersions[chart]} stable/${chart}`);
  }
  ...
  async _installMatrixbot() {
    await this._cmd(`upgrade --install --atomic matrixbot --set botUser=${this.botUser} --set botPassword=${this.botPassword} --set roomId=${this.botRoomId} --version ${this.mainConfig.chartVersions['matrixbot']} w3f/matrixbot`);
  }
  ...
  async _installGenericChart(chart, repo='stable') {
    ...
    await this._cmd(`upgrade --install --atomic ${chart} -f ${chartValuesPath} --version ${this.mainConfig.chartVersions[chart]} ${repo}/${chart}`);
  }
  ...
  async _installPolkadotSecrets() {
    ...
    this.files.writeYAML(fullValuesFilePath, values);
    const baseVersion = this.mainConfig.chartVersions.polkadot;
    await this._cmd(`upgrade --install --atomic polkadot-secrets -f ${relativeValuesFilePath} --version ${baseVersion} w3f/polkadot-secrets`);
    //await this._cmd(`upgrade --install --atomic polkadot-secrets -f ${relativeValuesFilePath} /home/fgimenez/workspace/w3f/polkadot-chart/charts/polkadot-secrets`);
  }
}
