...
class Helm {
  ...
  async _purgeCertManager(){
    const certManagerResources = this.kubectl.cmd(`kubectl get Issuers,ClusterIssuers,Certificates,CertificateRequests,Orders,Challenges --all-namespaces`) // | grep cert-manager
    certManagerResources.forEach(element => {
      console.log(element);
      let namespace = element //| awk '{print $1;}'
      let resource = element //| awk '{print $2;}'
      await this.kubectl.cmd(`kubectl -n ${namespace} delete ${resource}`);
    });

    const certManagerDeployment = this.kubectl.cmd(`kubectl -n cert-manager describe deployments.apps cert-manager`)
    const certManagerVersion = certManagerDeployment // | grep Image | cut -d':' -f3
    await this.kubectl.cmd(`kubectl delete -f https://github.com/jetstack/cert-manager/releases/download/v${certManagerVersion}/cert-manager.yaml`);


  }

  async _installCertManager() {
    await this.kubectl.cmd(`apply --validate=false -f https://github.com/jetstack/cert-manager/releases/download/v${this.mainConfig.chartVersions['cert-manager']}/cert-manager.crds.yaml`);
