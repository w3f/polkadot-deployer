...
class Helm {
  ...
  async _installPrometheusOperator() {
    if(this.opsgenieToken === ''){
      return this._installGenericChart('prometheus-operator');
    }
    else {
      await this._cmd(`upgrade --install prometheus-operator --set opsgenie.enabled=true --set opsgenie.token=${this.opsgenieToken} --set opsgenie.url=${this.opsgenieUrl} --version ${this.mainConfig.chartVersions['prometheus-operator']} stable/prometheus-operator`);
    }
  }
