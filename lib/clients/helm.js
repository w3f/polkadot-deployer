...
const INGRESS_NAMESPACES='ingress'
...
class Helm {
  ...
  async installDeps(index=0) {
    ...
    if (this.config.type !== 'local') {
      await this._installPolkadotSecretsGrafana(index);
  ...
  _generateValuesFileSecretsGrafana() {
    const values = {
      cloudflareSecret: {},
      grafanaSecret: {}
    };
  ...
  async _installNgixIngress() {
    return this._installGenericChart('ingress-nginx','center/kubernetes-ingress-nginx',INGRESS_NAMESPACES);
  }
  ...
  async _installExternalDNS() {
    ...
    return this._installTemplatedChart('external-dns', data, 'center/bitnami',INGRESS_NAMESPACES);
  }
  ...
  async _installGrafana() {
    return this._installGenericChart('grafana', 'center/grafana', MONITORING_NAMESPACE);
  }
  ...
  async _installGenericChart(chart, repo='stable', namespace='default') {
  ...
  async _installTemplatedChart(name, data, repo='stable', namespace='default', releaseName=name) {
  ...
  async _installSubstrateAlertrules() {
    ...
    return this._installTemplatedChart(chartName, data, 'w3f', this.namespace);
  }
  ...
  async _installPolkadotSecretsGrafana() {
    const values = this._generateValuesFileSecretsGrafana();
    const relativeValuesFilePath = this.files.relativeValuesFilePath(this.config.name, 'secretsGrafana');
    const fullValuesFilePath = this.files.absoluteValuesFilePath(this.config.name, 'secretsGrafana');
    this.files.writeYAML(fullValuesFilePath, values);
    const baseVersion = this.mainConfig.chartVersions.polkadot;

    const cmd = `upgrade --install polkadot-secrets-grafana -f ${relativeValuesFilePath} --version ${baseVersion} --namespace ${MONITORING_NAMESPACE} w3f/polkadot-secrets`
    console.log(cmd)
    await this._cmd(cmd);
  ...
  async _installPolkadotSecretsCloudflare() {
    ...
    const cmd = `upgrade --install polkadot-secrets-cloudflare -f ${relativeValuesFilePath} --version ${baseVersion} --namespace ${INGRESS_NAMESPACES} w3f/polkadot-secrets`
    console.log(cmd)
    await this._cmd(cmd);
    //await this._cmd(`upgrade --install polkadot-secrets -f ${relativeValuesFilePath} --namespace ${this.namespace} /home/fgimenez/workspace/w3f/polkadot-chart/charts/polkadot-secrets`);
  }
  ...
  async _ensureNamespacesInitialized() {
    const required = ['cert-manager',MONITORING_NAMESPACE,INGRESS_NAMESPACES];
