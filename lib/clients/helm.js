...
class Helm {
  ...
  _generateValuesFile(index) {
    ...
    if (index === 0) {
      ...
      if (this.config.type !== 'local') {
        ...
        values.domain = `${this.config.name}.${this.config.remote.domain}`
      }
    }
  ...
  async installDeps(namespace='default') {
    if (this.config.type === 'local') {
      return
    }

    await this._cmd('install -n nginx-ingress --set controller.publishService.enabled=true stable/nginx-ingress');
    ...
    await this._cmd('repo update');
    await this._cmd('install -n cert-manager --namespace cert-manager jetstack/cert-manager');

    await this._cmd(`install -n external-dns --set rbac.create=true --set provider=cloudflare --set cloudflare.apiKey=${this.cloudflareApiKey} --set cloudflare.email=${this.cloudflareEmail} --set cloudflare.proxied=false stable/external-dns`);
  }
}
