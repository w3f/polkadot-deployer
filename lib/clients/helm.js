...
class Helm {
  ...
  _generateValuesFile(index) {
    ...
    if (index === 0) {
      ...
      if (this.config.customChainspec) {
        values.createConfigMap = true;
        values.chainspec.minimumPeriod = this._minimumPeriod();
        values.chainspec.keys = {
          stash: this.keys.stash.map( item => item.address ),
          controller: this.keys.controller.map( item => item.address ),
          session: this.keys.session.map( item => item.address )
        }
      }
    }
    if (this.config.type !== 'local') {
