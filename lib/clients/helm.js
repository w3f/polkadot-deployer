...
class Helm {
  constructor(config) {
    ...
    this.opsgenieToken = process.env.OPSGENIE_TOKEN || 'opsgenie_token';
  ...
  async installDeps(index=0) {
    ...
    if (this.config.monitoring.enabled) {
      await this._installMonitoringStack();
    }
  ...
  async deleteDeps(index) {
    ...
    deps.forEach((dep) => {
      promises.push(this._cmd(`delete ${dep} --purge`));
    });
    return Promise.all(promises);
  }
  ...
  async install(){
    ...
    for (const n of Array(this.config.nodes).keys()) {
      ...
      try {
        ...
        //await this._cmd(`upgrade --install ${name} -f ${relativeValuesFilePath} /home/fgimenez/workspace/w3f/polkadot-chart/charts/polkadot`);
      } catch(e) {
  ...
  _generateValuesFile(index) {
    ...
    if (this.config.type !== 'local' && this.config.monitoring.enabled) {
  ...
  _generateValuesFileBase(index) {
    ...
    if (this.config.type === 'local' ) {
      ...
      values.monitoring = this.config.monitoring.enabled || false;
  ...
  _generateValuesFileSecrets() {
    ...
    if (this.config.type === 'local' ) {
      ...
      values.opsgenieSecret.create = true;  // todo: must chainge to false before merge
    } else {
      ...
      values.opsgenieSecret.create = true;
      values.opsgenieSecret.token = this.config.monitoring.opsgenie.token;
    }
    return values;
  }
  ...
  async _installCertManager() {
    await this.kubectl.cmd('apply --validate=false -f https://raw.githubusercontent.com/jetstack/cert-manager/release-0.11/deploy/manifests/00-crds.yaml');
    const nsList = await this.kubectl.cmd('get namespace');
    if(!nsList.toString().match(/cert-manager/)) {
      await this.kubectl.cmd(`create namespace cert-manager`);
    }
  ...
  async _installExternalDNS() {
    ...
    await this._cmd(`upgrade --install ${chart} --set cloudflare.apiKey=${this.cloudflareApiKey} --set cloudflare.email=${this.cloudflareEmail} -f ${chartValuesPath} --version ${this.mainConfig.chartVersions[chart]} stable/${chart}`);
  }

  async _installMonitoringStack() {
    //console.log(this.config.monitoring.opsgenie.enabled,  this.config.monitoring.opsgenie.token, this.config.monitoring.opsgenie.url);
  ...
  async _installPrometheusOperator() {
     try {
       const templatePath = "./config/charts/templates/prometheus-operator-opsgenie.tpl"
       ...
       const data = {
         ...
         opsgenieToken : this.config.monitoring.opsgenie.token,
         opsgenieUrl: this.config.monitoring.opsgenie.url
         }
       const prometheusConfig = template(data)
       fs.writeFile('./config/charts/prometheus-operator.yaml', prometheusConfig, (err) => {
           if (err) throw err;
       })
     } catch(e) {
         console.log('Error:', e.stack);
     }
     //await this._cmd(`upgrade --install prometheus-operator --set opsgenie.enabled=${this.config.monitoring.opsgenie.enabled} --set opsgenie.token=${this.config.monitoring.opsgenie.token} --set opsgenie.url=${this.config.monitoring.opsgenie.url} --version ${this.mainConfig.chartVersions['prometheus-operator']} stable/prometheus-operator`);
     await this._cmd(`upgrade --install prometheus-operator --version ${this.mainConfig.chartVersions['prometheus-operator']} stable/prometheus-operator`);
  ...
  async _installMatrixbot() {
    await this._cmd(`upgrade --install matrixbot --set botUser=${this.botUser} --set botPassword=${this.botPassword} --set roomId=${this.botRoomId} --set homeserver=${this.botHomeserver} --version ${this.mainConfig.chartVersions['matrixbot']} w3f/matrixbot`);
  }

  async _installSubstrateTelemetry() {
    const telemetryDomain = domain.default(this.config.name, this.config.remote);
    await this._cmd(`upgrade --install substrate-telemetry --set domain=${telemetryDomain} --set image.tag=${this.mainConfig.chartVersions['substrate-telemetry']} --version ${this.mainConfig.chartVersions['substrate-telemetry']} w3f/substrate-telemetry`);
  }
