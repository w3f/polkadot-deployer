...
class Helm {
  ...
  async _installPolkadotSecrets() {
    ...
    this.files.writeYAML(fullValuesFilePath, values);
    const baseVersion = this.mainConfig.chartVersions.polkadot;
    await this._cmd(`upgrade --install polkadot-secrets -f ${relativeValuesFilePath} --version ${baseVersion} --namespace ${this._namespace()} w3f/polkadot-secrets`);
  ...
  async _createNamespace(ns) {
    ...
    if(nsList.toString().match(/${ns}/)) {
      console.log(`namespace ${ns} not found`)
      return
    }
    console.log(`namespace ${ns} not found, creating`)
