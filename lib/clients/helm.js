...
class Helm {
  ...
  async _installPrometheusOperator() {
    if(this.config.opsgenieEnabled){
      await this._cmd(`upgrade --install prometheus-operator --set opsgenie.enabled=${this.config.opsgenie.enabled} --set opsgenie.token=${this.opsgenieToken} --set opsgenie.url=${this.config.opsgenie.url} --version ${this.mainConfig.chartVersions['prometheus-operator']} stable/prometheus-operator`);
    }
    else {
