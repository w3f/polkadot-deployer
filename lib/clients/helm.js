...
class Helm {
  ...
  async _installNgixIngress() {
    await this._cmd(`upgrade --install nginx-ingress --set controller.publishService.enabled=true --set controller.config.use-http2="false" --version ${this.mainConfig.chartVersions['nginx-ingress']} stable/nginx-ingress`);
  }
