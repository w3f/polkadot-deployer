...
class Helm {
  ...
  async installDeps(namespace='default') {
    ...
    if (this.config.type !== 'local' && !this.config.benchmark) {
    ...
    const values = this._generateValuesFileBase();
  ...
  _generateValuesFile(index) {
    const values = {
      ...
      mainNodeID: this.config.nodeKeys[0].peerId
    };
    if (index === 0) {
