...
class Helm {
  ...
  async _installCertManager() {
    await this.kubectl.cmd('apply -f https://raw.githubusercontent.com/jetstack/cert-manager/release-0.10/deploy/manifests/00-crds.yaml');
    await this.kubectl.cmd(`create namespace cert-manager`);
    await this.kubectl.cmd(`label namespace certmanager certmanager.k8s.io/disable-validation=true --overwrite=true`);
