...
class Helm {
  ...
  async _installExternalDNS() {
    files.copyChartValuesFiles(this.config.name, 'external-dns');
    const chartValuesPath = files.chartValuesPath(this.config.name, 'external-dns');

    await this._cmd(`install -n external-dns --set cloudflare.apiKey=${this.cloudflareApiKey} --set cloudflare.email=${this.cloudflareEmail} -f ${chartValuesPath} --version ${this.mainConfig.chartVersions['external-dns']} stable/external-dns`);
  }
}
