...
class Helm {
  ...
  async _installNgixIngress() {
    await this._cmd(`upgrade --install --atomic nginx-ingress --set controller.publishService.enabled=true -version ${this.mainConfig.chartVersions['nginx-ingress']} stable/nginx-ingress`);
  }
  ...
  async _installSubstrateTelemetry() {
    ...
    await this._cmd(`upgrade --install --atomic substrate-telemetry --set domain=${telemetryDomain} --set image.tag=${this.mainConfig.chartVersions['substrate-telemetry']} --version ${this.mainConfig.chartVersions['substrate-telemetry']} w3f/substrate-telemetry`);
  }
