...
class Helm {
  ...
  async install(){
    ...
    for (const n of Array(this.config.nodes).keys()) {
      ...
      const currentPromise = this.cmd(`install -n ${name} -f ${relativeValuesFilePath} --version ${chartVersion} ${chartName}`);
      if (n === 0) {
  ...
  async _ensureInitialized() {
    ...
    const matcherOptions = Object.assign({ matcher: /tiller-deploy-.*\s*1\/1\s*Running/m }, this.options);
    await cmd.exec(
