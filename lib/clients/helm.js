...
class Helm {
  ...
  async _installPrometheusOperator() {
    const source = path.join(this.files.projectConfigPath(), 'charts', 'templates', 'prometheus-operator.tpl');
    const target = path.join(this.files.projectConfigPath(), 'charts', 'prometheus-operator.yaml');
    //this.files.copyChartTemplates(this.config.name);
    //const target = path.join(this.valuesPath(), 'prometheus-operator.yaml');
    const data = {
    ...
    console.log(target)
    console.log("------------------------------------")
    await this._cmd(`upgrade --install prometheus-operator -f ${target} --version ${this.mainConfig.chartVersions['prometheus-operator']} stable/prometheus-operator`);
  }
