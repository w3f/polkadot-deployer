...
class Helm {
  ...
  async _installMonitoringStack(index) {
    ...
    await this._installSubstrateAlertrules(index);
  ...
  async _installSubstrateAlertrules(index) {
    const chartName = 'substrate-alertrules';
    const chartCompleteName = 'w3f/' + chartName;
    const chartVersion = this.mainConfig.chartVersions['substrate-alertrules'];
    const values = this._generateValuesFile(index);
    const relativeValuesFilePath = this.files.relativeValuesFilePath(this.config.name, index);
    const fullValuesFilePath = this.files.absoluteValuesFilePath(this.config.name, index);
    this.files.writeYAML(fullValuesFilePath, values);
    ...
    try {
      await this._cmd(`upgrade --install ${chartName} -f ${relativeValuesFilePath} --version ${chartVersion} --namespace ${this.namespace} ${chartCompleteName}`);
    } catch(e) {
      console.log(`Could not install ${chartName}: ${e.message}`);
    }
  }
