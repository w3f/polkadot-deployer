...
class Helm {
  ...
  async _installNgixIngress() {
    return this._installGenericChart('ingress-nginx','center/kubernetes-ingress-nginx');
  }
  ...
  async _installExternalDNS() {
    ...
    return this._installTemplatedChart('external-dns', data, 'center/bitnami');
  }
  ...
  async _installPrometheusOperator() {
    ...
    return this._installTemplatedChart('kube-prometheus-stack', data, 'center/prometheus-community','default','prometheus-operator');
  }

  async _installPrometheusNodeExporter() {
    return this._installGenericChart('prometheus-node-exporter', 'center/prometheus-community');
  }

  async _installGrafana() {
    return this._installGenericChart('grafana', 'center/grafana');
  }
  ...
  async _installTemplatedChart(name, data, repo='stable', namespace='default', releaseName=name) {
    ...
    await this._cmd(`upgrade --install ${releaseName} -f ${target} --version ${this.mainConfig.chartVersions[name]} --namespace ${namespace} ${repo}/${name}`);
  }
