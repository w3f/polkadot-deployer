...
class Helm {
  ...
  async _installCertManager() {
    ...
    try {
      await this._cmd(`upgrade --install cert-manager --namespace cert-manager --version ${this.mainConfig.chartVersions['cert-manager']} --set installCRDs=true jetstack/cert-manager`);
