...
module.exports = {
  ...
  readMainConfig: () => {
    const mainConfigPath = path.join(module.exports.projectConfigPath(), 'main.json');
    return module.exports.readJSON(mainConfigPath);
  },
  ...
  copyTerraformFiles: (name, type) => {
    ...
    module.exports.copy(source, target);

    const terraformBinSource = module.exports.terraformBinPath();
    const terraformBinTarget = path.join(target, 'terraform');

    module.exports.copy(terraformBinSource, terraformBinTarget);
  },
  chartValuesFileName: (chart) => {
    return `${chart}.yaml`;
  },
  chartValuesPath: (name, chart) => {
    const valuesFileName = module.exports.chartValuesFileName(chart);

    return path.join(module.exports.valuesPath(name), valuesFileName);
  },
  copyChartValuesFiles: (name, chart) => {
    const valuesFileName = module.exports.chartValuesFileName(chart);

    const source = path.join(module.exports.projectConfigPath(), 'charts', valuesFileName);

    if (!module.exports.fileExists(source)) {
      throw new Error(`File ${source} doesn't exist`);
    }

    const target = module.exports.chartValuesPath(name, chart);
