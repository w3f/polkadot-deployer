...
class Files {
  ...
  prometheusOperatorPath (name) {
    return path.join(this.deploymentPath(name), 'prometheusOperator');
  }

  prometheusOperatorTplPath(type) {
    return path.join(path.dirname(module.filename), '..', '..', 'prometheus-operator.tpl', type);
  }
